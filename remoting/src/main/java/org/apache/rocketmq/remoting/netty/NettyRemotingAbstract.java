...
public abstract class NettyRemotingAbstract {
    ...
    /**
     * 默认的请求处理器， 当requestCode没有找到具体的处理器的时候，采用该处理器
     ...
     *
     * @param permitsOneway 处理最大单向请求数量
     * @param permitsAsync  处理最大异步请求数量
     */
    ...
    public void processRequestCommand(final ChannelHandlerContext ctx, final RemotingCommand cmd) {
        final Pair<NettyRequestProcessor, ExecutorService> pair = this.processorTable.getOrDefault(cmd.getCode(), this.defaultRequestProcessor);
        /**
         * 获取请求唯一ID
         */
        ...
        if (pair != null) {
            ...
            Runnable run = () -> {
                try {
                    ...
                    final RemotingResponseCallback callback = response -> {
                        ...
                        /**
                         * one-way  不关心response 不进行response
                         */
                        if (!cmd.isOnewayRPC()) {
                            if (response != null) {
                                response.setOpaque(opaque);
                    ...
                    /**
                     * one-way  不关心response 不进行response， 其余同步或者异步， 则返回系统异常
                     */
                    if (!cmd.isOnewayRPC()) {
            ...
            try {
                ...
                /**
                 * one-way不关心response， 如果不是one-way则通知客户端，服务器过载
                 */
                if (!cmd.isOnewayRPC()) {
    ...
    /**
     * 用于定期扫描responseTable中超时的请求，移除并通知客户端
     */
    public void scanResponseTable() {
        final List<ResponseFuture> timeoutResponseList = new LinkedList<>();
        ...
        while (it.hasNext()) {
            ...
            /**
             * TODO 为什么加1000？
             * 判断请求是否超时，如果超时，从responseTable中移除，然后放到responseList李忠
             */
            if ((rep.getBeginTimestamp() + rep.getTimeoutMillis() + 1000) <= System.currentTimeMillis()) {
                rep.release();
                it.remove();
                timeoutResponseList.add(rep);
                log.warn("remove timeout request, " + rep);
            }
        }

        for (ResponseFuture rf : timeoutResponseList) {
            try {
